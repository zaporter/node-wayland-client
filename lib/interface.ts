...
import {
  ...
  InterfaceDefinition,
  isInterfaceArgument,
  isCallbackArgument as isCallbackArgument,
  isCallbackRequest,
  isInterfaceCreationRequest,
  isDestructorRequest,
} from "./definitions.js";
...
export default class Wl_interface extends EventEmitter {
  ...
  constructor(
    d: Display,
    id: number,
    { name, version, requests, events, enums }: InterfaceDefinition,
  ) {
    super();
    this.display = d;
    this.id = id;
      ...
      if (isCallbackRequest(op)) {
        ...
        (this as any)[op.name] = async (...args: any[]) => {
          ...
          try {
            ...
            throw new Error(
              `${this.name}.${op.name}(${op.args.map(({ name }) => name).join(", ")}) failed: ${e.message}`,
            );
          }
          return itf;
        };
        ...
        (this as any)[op.name] = this.display.request.bind(
          this.display,
          this.id,
          opcode,
          op,
  ...
  public push(evcode: number, b: Buffer): boolean {
    ...
    if (!event)
      return this.emitError(
        new Error(`No event with index ${evcode} in interface ${this.name}`),
      );
    try {
      ...
      if (isInterfaceArgument(event.args[0])) {
        if (values.length != 1) {
          ...
          console.warn(
            "Found an interface creation event that had more than one argument. This is unexpected.",
          );
        }
        const [id] = values;
  ...
  opcode(rName: RequestDefinition["name"]) {
    ...
    if (opcode == -1)
      throw new Error(`no operation named ${rName} in interface ${this.name}`);
    return opcode;
  }
  ...
  inspect() {
    return (
      ...
      this.requests
        .map(
          (r) =>
            `  ${r.name}(${r.args.map((a) => `${a.name} :${a.type}`).join(", ")})`,
        )
        .join("\n")
    );
  }
  ...
  aggregate(): () => AggregateResult {
    ...
    return function end_aggregate() {
      ...
      for (let key in infos) {
        if ((infos[key] as any).length == 1)
          infos[key] = (infos[key] as any)[0];
      }
      return infos;
    };
  }
   ...
   */
  async drain(
    until: Promise<any> = this.display.sync(),
