...
export default class Display extends EventEmitter {
  ...
  async init(): Promise<Wl_registry> {
     ...
     */
    (
      this.#interfaces.get("wl_registry") as any as Wl_interface
    ).requests[0].args = [
      {
        name: "id",
        type: "uint",
    ...
    wl_display.on("error", (srcId, errno, msg) => {
      ...
      if (srcErrors) {
        ...
        console.warn(
          "FATAL ERROR in %s: %s (undefiend error)",
          src.name,
          errno,
          msg,
        );
  ...
  protected onMessage(id: number, evcode: number, msg: Buffer) {
    ...
    if (!event)
      return this.emit(
        "warning",
        `interface ${target.name} has no event with index ${evcode}`,
      );
      ...
      typeof file === "string"
        ? await (async () => {
            if (/\.xml$/i.test(file)) {
              ...
              const {
                protocol: { interface: interfaces },
              } = xml2js(xml, { compact: true }) as ElementCompact;
              return interfaces.map(parseInterface);
            } else {
              if (!/\.json$/i.test(file))
                file = path.join(outDir, file + ".json");
   ...
   */
  async bind<T extends Wl_interface = Wl_interface>(
    iname: T["name"],
    version?: T["version"],
  ): Promise<T> {
    ...
    if (!gid)
      throw new Error(
        ` No global named ${iname}. Available globals : ${[...this.#globals.keys()].join(", ")}`,
      );
    if (typeof version !== "undefined" && version != def.version) {
      console.warn(
        `Version mismatch: user requests ${iname} v${version}, but v${def.version} is defined`,
      );
    }
   ...
   */
  registerInterface<T extends Wl_interface = Wl_interface>(
    id: number,
    name: string,
  ): T {
