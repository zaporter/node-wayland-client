...
export const readUInt = (
  ...
  offset: number,
  en: "LE" | "BE" = os_en,
): number => (en == "LE" ? b.readUInt32LE : b.readUInt32BE).call(b, offset);

/**
 * write an uint32 using native system's endianess
 */
export const writeUInt = (
  b: Buffer,
  value: number,
  offset: number,
  en: "LE" | "BE" = os_en,
): number =>
  (en == "LE" ? b.writeUInt32LE : b.writeUInt32BE).call(b, value, offset);

/**
 * read an int32 using native system's endianess
 */
export const readInt = (b: Buffer, offset: number, en: "LE" | "BE" = os_en) =>
  (en == "LE" ? b.readInt32LE : b.readInt32BE).call(b, offset);
...
export const writeInt = (
  ...
  (en == "LE" ? b.writeInt32LE : b.writeInt32BE).call(b, value, offset);

/**
 * write a 24.8 fixed point value.
 */
export function writeFixed(
  b: Buffer,
  value: number,
  offset: number,
  en: "LE" | "BE" = os_en,
...
export function format_args(args: any[], def: ArgumentDefinition[]): Buffer {
  if (args.length != def.length)
    throw new Error(
      `Bad number of arguments (${args.length}, expected ${def.length}).`,
    );
    ...
    switch (type) {
      ...
      case "uint":
        ...
        if (Number.isNaN(arg) || arg < 0)
          throw new Error(`Invalid ${type} value: ${arg}`);
