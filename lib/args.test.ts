...
describe("format_args()", function () {
  it("throws if the number of arguments is incorrect", function () {
    ...
    expect(() => format_args(args, def)).to.throw(
      Error,
      "Bad number of arguments (3, expected 2).",
    );
  });
  ...
  it("throws if argument type is unknown or unsupported", function () {
    ...
    ).to.throw(`Unsupported request argument type : foo`);
    //fd exists but is not supported
    expect(() =>
      format_args([1], [{ name: "arg1", type: "fd" } as any]),
    ).to.throw(`Unsupported request argument type : fd`);
  });
  ...
  ["int", "uint", "fixed", "enum", "string", "array"].forEach((type) => {
    [undefined, null, {}].forEach((arg) => {
      it(`throw if an argument for ${type} is ${arg}`, function () {
        const args = [arg];
        const def: ArgumentDefinition[] = [
          { name: "arg1", type: type as ArgumentType },
        ];
        expect(() => format_args(args, def)).to.throw(
          `Invalid type: ${typeof arg} for arg1. Expected a ${type}`,
        );
      });
    });
  });
...
describe("get_args()", function () {
  ["uint", "int", "object", "new_id", "enum"].forEach((type) => {
    it(`get ${type} argument`, function () {
      const b = Buffer.alloc(4);
      writeUInt(b, 1, 0);
      expect(get_args(b, [{ name: "arg1", type: type as any }])).to.deep.equal([
        1,
      ]);
    });
  });
  ...
  it(`get string argument`, function () {
    ...
    expect(get_args(b, [{ name: "arg1", type: "string" }])).to.deep.equal([
      "hello world",
    ]);
  });
  ...
  it("throws on bad definition", function () {
    expect(() =>
      get_args(Buffer.alloc(4), [{ name: "arg1", type: "foo" } as any]),
    ).to.throw("Unsupported event argument type : foo");
  });
