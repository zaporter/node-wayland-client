...
function has(
  key: string,
  o: ElementCompact,
): o is ElementCompact & { _attributes: { [k: typeof key]: string } } {
...
export function parseEnums(enums: ElementCompact | ElementCompact[]) {
  ...
  for (let en of Array.isArray(enums) ? enums : [enums]) {
    ...
    out[name] = to_a(entry, (e: ElementCompact): EnumValue => {
      ...
      const {
        _attributes: { name, value, summary },
      } = e;
...
export function parseInterface(itf: InterfaceElement): InterfaceDefinition {
  const {
    _attributes: { name, version: versionString },
    description,
    request,
    event,
    enum: enums,
  ...
  const version =
    typeof versionString === "number" ? versionString : parseInt(versionString);
  if (Number.isNaN(version))
    throw new Error(`Invalid version ${versionString} for interface ${name}`);
  return {
    name,
    version,
    description: description?._text,
